...
```csv
speaker,voice,text,tts_text,speed,style,enabled
...
|---|---|
| `speaker` | 角色显示名 |
| `voice` | 对应 `voices/` 中文件名（无扩展名） |
| `text` | 源语言原文 |
| `tts_text` | 配音用文本（译文 / 本土化结果）；空则回退 `text` |
| `speed` | 语速，默认 `1.0` |
| `style` | 可选风格提示；空则用全局默认 |
| `enabled` | 是否参与生成 |

也支持中文表头：`角色,音色,原文/文本/台词,配音文本/译文,语速,风格,启用`。
